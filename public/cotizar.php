        ...
        <div class="rounded-xl border <?= $isSel?'border-emerald-400 ring-2 ring-emerald-200':'' ?> p-4">
          ...
          <div class="mt-3 text-sm">
            <div class="flex items-center justify-between">
              <span class="text-gray-500">Costo papel</span>
            ...
            <?php else: ?>
              <div class="flex items-center justify-between">
                ...
                <span class="font-medium"><?= price($o['costos']['clicks'] ?? 0) ?></span>
              </div>
            <?php endif; ?>
            <div class="flex items-center justify-between">
...
$acabados = $repo->all(true);

// Defaults de form (GET inicial)
$defaults = [
  'tipo' => 'revista',
  'margen' => '0.30',
  // cover
...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
  // Construir specs por parte
  $margen = (float)float_input($F['margen'], 0.30);

  // Acabados elegidos por parte (IDs)
  $cov_fin_ids = array_map('intval', $_POST['cover_fin'] ?? []);
  $int_fin_ids = array_map('intval', $_POST['int_fin'] ?? []);
  $ins_fin_ids = array_map('intval', $_POST['ins_fin'] ?? []);

  $cov_fin_specs = $repo->asSpecs($cov_fin_ids);
  $int_fin_specs = $repo->asSpecs($int_fin_ids);
  $ins_fin_specs = $repo->asSpecs($ins_fin_ids);
  ...
  // Ladder opcional (si activas el checkbox)
